...
def _plot_tree_depth(corpus_scores: dict, title: str, output_path: Path) -> None:
...
def main() -> None:
    extractor = lambda cas: (
        SE_TreeStructureAnnotator().process(cas) ,
        FE_CasToTree().extract(cas)
    ...
    def compute_avg(scores_dict: dict[str, list[float]]) -> dict[str, float]:
        return {name: (mean(scores_dict.get(name, [])) if scores_dict.get(name) else float('nan')) for name in
                feature_names}
